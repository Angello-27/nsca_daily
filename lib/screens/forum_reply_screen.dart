...
import '../models/forum_questions_model.dart';
...
class _ForumReplyScreenState extends State<ForumReplyScreen> {
  ...
  void didChangeDependencies() {
    if (_isInit) {
      ...
      Provider.of<CourseForum>(
        context,
        listen: false,
      ).fetchForumChildQuestion(widget.questionId).then((_) {
        setState(() {
          _isLoading = false;
          activeQuestions =
  ...
  Future<void> _reply() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse(url),
        body: {'auth_token': token, 'description': text},
      );
                      ...
                      ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          var cmDt = DateTime.fromMillisecondsSinceEpoch(
                            int.parse(
                                  activeQuestions[index].dateAdded.toString(),
                                ) *
                                1000,
                          );
                          // 12 Hour format:
                          var commentDate = DateFormat(
                            'dd MMM, yy',
                          ).format(cmDt);
                          return Padding(
                                      ...
                                      child: Align(
                                        ...
                                        child: CircleAvatar(
                                          ...
                                          backgroundImage: NetworkImage(
                                            activeQuestions[index].userImage
                                                .toString(),
                                          ),
                                          backgroundColor: Colors.grey,
                                        ),
