...
class _SearchForumState extends State<SearchForum> {
  ...
  void didChangeDependencies() {
    if (_isInit) {
      ...
      Provider.of<CourseForum>(
        context,
        listen: false,
      ).fetchCourseSearchQuestions(widget.courseId, widget.keyWord).then((_) {
        setState(() {
          _isLoading = false;
          activeQuestions =
  ...
  reset() async {
    ...
    await Provider.of<CourseForum>(
      context,
      listen: false,
    ).fetchCourseSearchQuestions(widget.courseId, _controller.text).then((_) {
      setState(() {
        _isLoading = false;
        activeQuestions =
            Provider.of<CourseForum>(context, listen: false).questions;
